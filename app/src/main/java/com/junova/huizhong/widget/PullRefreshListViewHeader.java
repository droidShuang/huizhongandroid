...
public class PullRefreshListViewHeader extends LinearLayout {
	...
	private void initView(Context context) {
		...
		mContainer = (LinearLayout) LayoutInflater.from(context).inflate(
				R.layout.common_pull_refresh_listview_header, null);
	...
	public void setState(int state) {
		...
		if (state == STATE_REFRESHING) {//show loading
		...
		switch(state){
			case STATE_NORMAL:
				...
				mHintTextView.setText(R.string.push_down_refresh);
				break;
			case STATE_READY:
				if (mState != STATE_READY) {
					mArrowImageView.clearAnimation();
					mArrowImageView.startAnimation(mRotateUpAnim);
					mHintTextView.setText(R.string.release_refresh_data);
				}
				break;
			case STATE_REFRESHING:
				mHintTextView.setText(R.string.hint_loading);
				break;
				default:
		}
		mState = state;
